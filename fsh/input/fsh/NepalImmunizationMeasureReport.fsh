...
* measure only canonical(NepalImmunizationMeasure)

// Enforce that this is a summary report (aggregate data), not an individual patient report
* type = #summary

// Enforce the Geography Rule: It must be for a Geographic Location (District) or an Organization (Health Facility)
* subject 1..1
* subject only Reference(Location or Organization)
